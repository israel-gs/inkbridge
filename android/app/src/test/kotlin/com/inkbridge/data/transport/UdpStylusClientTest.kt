...
class UdpStylusClientTest {
     ...
     */
    @Test
    fun `sequential sends with reused buffer deliver correct distinct payloads`() = runBlocking {
        val server = DatagramSocket(0)
        server.soTimeout = 2_000
        val port = server.localPort
        val client = UdpStylusClient("127.0.0.1", port)
     ...
     */
    @Test
    fun `send with payload larger than reuse buffer uses fallback allocation`() = runBlocking {
        val server = DatagramSocket(0)
        server.soTimeout = 2_000
        val port = server.localPort
        val client = UdpStylusClient("127.0.0.1", port)
