...
class TcpStylusClientTest {
    ...
    @Test
    fun `send transmits bytes that are received by the server`() =
        runBlocking {
    ...
    @Test
    fun `connect to refused port returns failure`() =
        runBlocking {
     ...
     */
    @Test
    fun `send failure sets isConnected to false and emits to errors`() =
        runBlocking {
            val server = ServerSocket(0)
            val port = server.localPort
