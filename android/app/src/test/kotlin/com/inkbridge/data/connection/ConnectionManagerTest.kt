...
class ConnectionManagerTest {
    ...
    private fun makeManager(
        ...
        val factory =
            ConnectionManager.TransportFactory { host, port, _ ->
    ...
    @Test
    fun `failed connect transitions to Error`() =
        runBlocking {
    ...
    @Test
    fun `disconnect from Connected transitions to Disconnected`() =
        runBlocking {
            val fake = FakeTransport(Result.success(Unit))
            val manager = makeManager(fake)
    ...
    @Test
    fun `disconnect calls close on active transport`() =
        runBlocking {
            val fake = FakeTransport(Result.success(Unit))
            val manager = makeManager(fake)
    ...
    @Test
    fun `currentTransport is non-null after successful connect`() =
        runBlocking {
            val fake = FakeTransport(Result.success(Unit))
            val manager = makeManager(fake)
            manager.connect("192.168.1.1", 4545, TransportKind.WIFI_UDP)
    ...
    @Test
    fun `currentTransport is null after disconnect`() =
        runBlocking {
            val fake = FakeTransport(Result.success(Unit))
            val manager = makeManager(fake)
            manager.connect("192.168.1.1", 4545, TransportKind.WIFI_UDP)
    ...
    @Test
    fun `USB_TCP connect uses 127,0,0,1 regardless of provided host`() =
        runBlocking {
            val fake = FakeTransport(Result.success(Unit))
            val manager = makeManager(fake)
    ...
    @Test
    fun `WIFI_UDP connect uses the provided host`() =
        runBlocking {
            val fake = FakeTransport(Result.success(Unit))
            val manager = makeManager(fake)
    ...
    @Test
    fun `failed connect closes the transport`() =
        runBlocking {
            val fake = FakeTransport(Result.failure(Exception("refused")))
            val manager = makeManager(fake)
    ...
    @Test
    fun `transport error after connect transitions state to Error`() =
        runTest(UnconfinedTestDispatcher()) {
            ...
            fake.mutableErrors.emit(java.io.IOException("Connection reset by peer"))
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `transport error after disconnect does not re-enter Error state`() =
        runTest(UnconfinedTestDispatcher()) {
            val fake = FakeTransport(Result.success(Unit))
            val testScope = TestScope(UnconfinedTestDispatcher())
            val factory = ConnectionManager.TransportFactory { _, _, _ -> fake }
            ...
            fake.mutableErrors.tryEmit(java.io.IOException("Stale error"))
            testScope.advanceUntilIdle()
