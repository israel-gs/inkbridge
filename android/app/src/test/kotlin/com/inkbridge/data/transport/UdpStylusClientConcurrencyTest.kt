...
class UdpStylusClientConcurrencyTest {
    ...
    @Test
    fun `concurrent sends from 4 coroutines all deliver correct bytes`() = runBlocking {
    ...
    @Test
    fun `concurrent sends produce no corrupted datagrams`() = runBlocking {
        val server = DatagramSocket(0)
        server.soTimeout = 5_000
        val port = server.localPort
        val client = UdpStylusClient("127.0.0.1", port)
        ...
        repeat(4) {
            ...
            assertEquals(1, unique.size, "Datagram must not contain mixed bytes from concurrent sends; found: $unique")
        }
