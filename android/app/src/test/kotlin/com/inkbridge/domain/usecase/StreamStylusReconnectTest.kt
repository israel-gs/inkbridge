...
class StreamStylusReconnectTest {
    ...
    private fun sample(i: Int = 0) = StylusSample(
        x = 0.5f, y = 0.5f, pressure = 32767, tiltX = 0, tiltY = 0,
        hover = false, timestampNs = i.toLong(),
    )

    // ── Reconnect routing ─────────────────────────────────────────────────────

    @Test
    fun `pre-swap frames reach transport1, post-swap frames reach transport2`() = runBlocking {
    ...
    @Test
    fun `sentCount is cumulative across transport swaps`() = runBlocking {
        val t1 = RecordingTransport()
        val t2 = RecordingTransport()
        val useCase = StreamStylus(transport = t1)
    ...
    @Test
    fun `swapTransport to null makes frames drop gracefully`() = runBlocking {
        val t1 = RecordingTransport()
        val useCase = StreamStylus(transport = t1)
    ...
    @Test
    fun `swapTransport replaces failing transport and subsequent sends succeed`() = runBlocking {
     ...
     */
    @Test
    fun `first emit after swapTransport reaches new transport immediately`() = runBlocking {
        val t1 = RecordingTransport()
        val t2 = RecordingTransport()
        val useCase = StreamStylus(transport = t1)
