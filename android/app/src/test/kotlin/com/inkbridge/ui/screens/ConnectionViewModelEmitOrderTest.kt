...
class ConnectionViewModelEmitOrderTest {
     ...
     */
    @Test
    fun `channel consumer delivers sample before button for ACTION_DOWN`() = runTest {
        ...
        assertTrue(sink.calls[1].startsWith("button(primary=true)"), "Second call must be button-down, got: ${sink.calls[1]}")
    }
     ...
     */
    @Test
    fun `100 rapid channel sends arrive in order`() = runTest {
        val sink = RecordingSink()
        val channel = Channel<suspend (StylusSink) -> Unit>(Channel.UNLIMITED)
     ...
     */
    @Test
    fun `swapTransport mid-stroke routes post-swap frames to new transport`() = runTest {
     ...
     */
    @Test
    fun `first MOVE after DOWN preserves all historical samples`() = runTest {
        val sink = RecordingSink()
        val channel = Channel<suspend (StylusSink) -> Unit>(Channel.UNLIMITED)
